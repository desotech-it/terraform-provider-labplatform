...
resource "labplatform_class" "cka" {
...
resource "labplatform_class_student" "cka" {
  for_each = labplatform_user.cka_students

  class_id   = labplatform_class.cka.id
...
resource "labplatform_class" "docker" {
...
resource "labplatform_class_student" "docker" {
  for_each = labplatform_user.docker_students

  class_id   = labplatform_class.docker.id
  user_id      = each.value.id
  template_ids = [local.vnc_template.id, local.ssh_template.id]
}
